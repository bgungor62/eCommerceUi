...
import { getMyCart, type CartItemDto } from "../api/cart";
...
export default function Navbar() {
  ...
  const cartCount =
    cart?.items?.reduce((sum: number, i: CartItemDto) => sum + (i.quantity ?? 0), 0) ?? 0;
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
        <Link to="/products" className="flex items-center gap-2">
          ...
          <div className="leading-tight">
            <div className="font-semibold text-slate-900">ECommerce</div>
            <div className="text-xs text-slate-500">storeFront</div>
          </div>
        </Link>
